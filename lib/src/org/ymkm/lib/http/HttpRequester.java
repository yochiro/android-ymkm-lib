 ...
 * <ul>
 * <li>
 * Synchronous request (runs on the caller thread, {@link android.os.NetworkOnMainThreadException} will be raised if ran
 * from the UI thread!). Note that only GET is available for this type of operation, as POST/PUT requests most likely
 * require additional request parameters to be set, while this method does not has any.
 * <ul>
 * <li>
 ...
 * 
 * <pre>
 * HttpRequester.requestGET(new HttpRequester.HttpParamGet(&quot;http://remote.url/path/&quot;));
 * </pre>
 * 
 * </p>
 * </li>
 ...
 * {@link HttpParamGet} ({@link HttpParamPost}, {@link HttpParamPut} for POST/PUT requests) is used to setup parameters
 * to be appended to the URI before the request :
 * <p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * For POST/PUT requests, parameters added using {@code addParam} will be added to the query string the same way as GET
 * requests does. To set the body content, use {@linkplain HttpRequestListener#onRequestPrepare(ExecutableRequest, Uri)}
 * as described below.<br>
 * In essence, {@link HttpParamPost#setEntity(org.apache.http.HttpEntity)} should be used (or HttpParamPut for put). In
 * addition, header should be set with the correct {@code Content-Type}, optionally {@code Accept}. <br>
 ...
 * </li>
 * <li>
 * Asynchronous requests (a new thread will be spawned, processing the request in the background).
 * <ul>
 * <li>
 * <p>
 * Basic usage is no different than {@code HttpRequester#requestGET}.<br/>
 * Without any other parameter, asynchronous request response will not be processable.<br/>
 * Use {@code HttpRequester.requestGETAsync} instead : ( {@code HttpRequester#requestPOSTAsync}/
 * {@code HttpRequester#requestPUTAsync} can be used for resp. POST and PUT requests).
 * </p>
 * 
 * <p>
 * 
 ...
 * </pre>
 * 
 * Note: this version is only available for GET, as it doesn't allow parameters to be set.
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * Additionally, {@code HttpRequester.requestGETAsync} can take a {@link HttpRequestListener} parameter, allowing to
 * perform some operations before the request is executed, and to process the server response :
 * </p>
 * 
 * <p>
 * 
 ...
 * </pre>
 * 
 * HttpRequestListener can override two methods that runs before/after the request :<br/>
 ...
 * 
 * <p>
 * Also, a third parameter, the calling {@link Activity} can be passed : if not null, and if available then the
 * indeterminate progress bar visibility will be switched based on the request execution status.
 * </p>
 * <br>
 * <p>
 * {@link HttpRequestListener#onRequestPrepare(ExecutableRequest, Uri)} can be used in POST/PUT requests to set the
 * content body, additional headers... <br>
 * <em>Example:</em>
 * 
 * <pre>
 * <code>
 ...
 * </pre>
 * 
 * Alternatively, helper methods are provided in {@linkplain HttpRequestListener} to set request body as a urlencoded
 * form entity or json entity.<br>
 * See {@linkplain HttpRequestListener#setUrlEncodedFormEntity(PostableRequest, List)} or
 * {@linkplain HttpRequestListener#setJsonEntity(PostableRequest, String)}. <br>
 * For more complex content, the developer should implement its own logic.
 * </p>
 * <br>
 * <p>
 * Note that all parameters added using addParam* method will only be added as a query string for the request. The body
 * content should be handled by the developer using the way described above.
 * </p>
 * 
 * </li>
 * </ul>
 * </li>
 * <li>
 * {@link HttpRequester#newRequest} can also be used to create the correct {@link ExecutableRequest} based on specified
 * request type. In that case, {@code .execute(HttpRequestListener)} may be used instead to execute the network
...
public final class HttpRequester {
	...
	/**
	 * Base class for a request listener when running asynchronous requests through
	 * {@link HttpRequester.requestGETAsync}.
	 * <p>
	 * If passed as a parameter to the latter, {@linkplain HttpRequestListener.onRequestPrepare} will be called just
	 * before the request is executed, while {@linkplain HttpRequestListener.onRequestComplete} will be called with the
	 * server response as its parameter.<br/>
	 * Both methods are called within the caller thread context (e.g. the main UI thread)
	 * </p>
	 * <p>
	 * Additionally, the following methods can be overridden :<br/>
	 * {@link HttpRequestListener.onSetHttpParams} will be called before executing the request, passing the HttpParams
	 * to set to the HttpClient. Caller can add HTTP parameters as necessary.
	 * {@link HttpRequestListener.onSetHttpClient} will be called before executing the request, passing the HttpClient
	 * that will be used to execute the request. Caller can modify its behavior as necessary.
	 * </p>
	 * <p>
	 * Server response is encapsulated within an {@link HttpResponseHolder} object, which is a simple wrapper of
	 * {@link ord.apache.http.HttpResponse} .
	 * </p>
	 * 
	 * @author mikami-yoann
	 */
	public static abstract class HttpRequestListener {
		...
		/**
		 * Called on created HttpClient instance to allow custom config by caller
		 * 
		 * @param client
		...
		/**
		 * Called on a default set of HTTP parameters to allow customization by caller
		 * 
		 * @param params
		...
		/**
		 * Called before the request is executed. Wrapper for abstract method onRequestPrepare. <br/>
		 * Can be used to perform pre/post initializations around onRequestPrepare.
		 * 
		 * @param http
		...
		/**
		 * Called after the request is executed. Wrapper for abstract method onRequestComplete. <br/>
		 * Can be used to perform pre/post initializations around onRequestComplete.
		 * 
		 * @param httpGet
		...
		/**
		 * Called if the request was cancelled by the caller. Wrapper for abstract method onRequestCancelled. <br/>
		 * 
		 */
		 ...
		 * 
		 * @param pr
		 *            the PostableRequest (HttpParamPost, HttpParamPut) to set form entity on
		 * @param params
		 *            parameter list to urlencode as content body
		 */
		 ...
		 * 
		 * @param pr
		 *            the PostableRequest (HttpParamPost, HttpParamGet) to set json entity on
		 * @param jobj
		...
		/**
		 * Gives an opportunity for the caller to modify client to be used by request before its execution. Contract:
		 * {@code execute} should not be called in this method!!
		 * 
		 * @param params
		...
		/**
		 * Gives an opportunity for the caller to set specific HTTP parameters before executing the request
		 * 
		 * methods defined in {@link HttpConnectionParams}, {@link ConnManagerParams}, {@link HttpProtocolParams} can be
		 * used on the parameter to customize the HTTP request.
		 * 
		 * @param params
		...
		/**
		 * Subclasses may override this. Callback called before the request is made. <br/>
		 * Does nothing by default.
		 * <p>
		 * Subclasses may want to override this to add parameters, headers to the ExecutableRequest object, or update
		 * the UI.<br>
		 * </p>
		 * <p>
		 ...
		 * </p>
		 * <p>
		 * <strong>Warning :</strong> This is called in the caller thread, so it should *NOT* perform heavy processing.
		 * </p>
		 * 
		 * @param http
		 *            the request about to be executed
		...
		/**
		 * Subclasses may override this. Callback called after the request is made. <br/>
		 * Does nothing by default.
		 * <p>
		 * Subclasses may want to override this to e.g. check response code, potential errors, or to retrieve the
		 * response body and react accordingly.<br/>
		 * <strong>Warning :</strong> This is called in the caller thread, so it should *NOT* perform heavy processing.
		 * </p>
		 * <p>
		 * The {@link HttpResponseHolder} parameter contains the HTTP status code in {@code returnCode}, the uri that
		 * was requested in {@code uri} , the response headers in {@code headers} and the response body in
		 * {@code responseBody}.
		 * </p>
		 * 
	 ...
	 * 
	 * GET, POST and PUT requests are all wrapped around this common interface. It subclasses {@link HttpUriRequest} and
	 * as such extends the built-in java interfaces.
	 * <p>
	 * {@link HttpParamGet}, {@link HttpParamPost}, {@link HttpParamPut} all implements this interface.
	 * </p>
	 */
	public static interface ExecutableRequest extends HttpUriRequest {
		 ...
		 * <p>
		 * If parameters were previously set using any of the {@code addParam} methods, the returned URI will be
		 * appended with a query string built using the list of parameters that were provided at this point.
		 * </p>
		 * 
		 * @return the URI to associated with current HTTP request, appended with the query string if any
		 */
		@Override
		URI getURI();
		 ...
		 * 
		 * @param whenLoaded
		 *            the {@linkplain HttpRequestListener} listener to use during the asynchronous operation
		 */
		void execute(HttpRequestListener whenLoaded);
	}

	/**
	 * Extension of an ExecutableRequest for requests that allow a body content to be sent alongwith the request.
	 * 
	...
	/**
	 * Returns a new {@code ExecutableRequest} instance which can be used to make an asynchronous HTTP request
	 * 
	 * Usage sample for a GET request :
	 * <p>
	 * 
	 * <pre>
	 * ExecutableRequest req = HttpRequester.newRequest(HttpRequester.REQUEST_GET, &quot;http://my/site/to/get&quot;);
	 ...
	 * 
	 * </p>
	 * The same can be achieved for a POST by replacing {@code HttpRequester.REQUEST_GET} with
	 * {@code HttpRequester.REQUEST_POST} .
	 * 
	 * HttpRequest
	 * 
	 * @param type
	 *            One of {@link HttpRequester#REQUEST_GET}, {@link HttpRequester#REQUEST_POST},
	 *            {@link HttpRequester#REQUEST_PUT}, {@link HttpRequester#REQUEST_DELETE},
	 ...
	 * @throws Exception
	 */
	public static ExecutableRequest newRequest(final int type, final String uri) throws Exception {
		switch (type) {
			case REQUEST_GET:
	...
	/**
	 * Extends HttpPost with an easy way to add POST query parameters to the request.
	 * <p>
	 * The added parameters will make it into the requested url as {@code uri?param1=xxx&param2=xxx...&paramn=xxx}.<br/>
	...
	public final static class HttpParamPost extends HttpPost implements PostableRequest, ParametrableEntity {
		 ...
		 * Returns the URI this request uses, such as {@code http://example.org/path/to/file}.
		 * 
		 * @return the URI to associated with current HTTP request, appended with the query string generated from the
		 *         added parameters if any
		 */
		@Override
		public URI getURI() {
	...
	/**
	 * Extends HttpPut with an easy way to add PUT query parameters to the request.
	 * <p>
	 * The added parameters will make it into the requested url as {@code uri?param1=xxx&param2=xxx...&paramn=xxx}.<br/>
	 * The parameters from the resulting URL are URL-encoded.
	 * </p>
	...
	/**
	 * Extends HttpGet with an easy way to add GET query parameters to the request.
	 * <p>
	 * The added parameters will make it into the requested url as {@code uri?param1=xxx&param2=xxx...&paramn=xxx}.<br/>
	 * The parameters from the resulting URL are URL-encoded.
	 * </p>
	...
	/**
	 * Returns a basic auth header on specified {@link ExecutableRequest} encoded with user/pass
	 * 
	 * @param user
	 *            the basic authorization username
	 * @param pass
	...
	public static Header getBasicAuthHeader(String user, String pass) {
		...
		String auth = "Basic "
				+ org.ymkm.lib.util.Base64.encodeToString(basicAuth.getBytes(), org.ymkm.lib.util.Base64.NO_WRAP);
	 ...
	 * - {@code String responseBody} : the response body returned by the server, as a single String<br>
	 * - {@code byte[] responseBytes} : the response body returned by the server, in bytes for binary responses<br>
	 ...
	 * </p>
	 * Using this method, the server response will not be given back to the caller.
	 * <p>
	 * Pass in a {@link HttpRequester.HttpRequestListener} to this method to allow the caller to process the response
	 ...
	 * This method spawns a new thread to perform the request
	 * </p>
	 * Using this method, the server response will not be given back to the caller.<br/>
	 * Also, HTTP params and the HTTP client cannot be customized using this variant of the method.
	 * <p>
	 * Pass in a {@link HttpRequester.HttpRequestListener} to this method to allow the caller to process the response
	 * returned by the server
	 * </p>
	 ...
	 * {@code responseBody} will contain the exception message.
	 * </p>
	 * 
	 * @param uri
	 *            the uri to request
	 ...
	 * @return HttpRequesterFuture a future object tied to this request
	 */
	public static HttpRequesterFuture requestGETAsync(final String uri, final HttpRequestListener whenLoaded) {
	 ...
	 * @return HttpRequesterFuture a future object tied to this request
	 */
	public static HttpRequesterFuture requestGETAsync(final HttpParamGet req, final HttpRequestListener whenLoaded) {
	 ...
	 * @return HttpRequesterFuture a future object tied to this request
	 */
	public static HttpRequesterFuture requestPOSTAsync(final String uri, final HttpRequestListener whenLoaded) {
	 ...
	 * @return HttpRequesterFuture a future object tied to this request
	 */
	public static HttpRequesterFuture requestPOSTAsync(final HttpParamPost req, final HttpRequestListener whenLoaded) {
		return requestAsync(req, whenLoaded, null);
	}
	 ...
	 * @return HttpRequesterFuture a future object tied to this request
	 */
	public static HttpRequesterFuture requestPUTAsync(final String uri, final HttpRequestListener whenLoaded) {
	 ...
	 * @return HttpRequesterFuture a future object tied to this request
	 */
	public static HttpRequesterFuture requestPUTAsync(final HttpParamPut req, final HttpRequestListener whenLoaded) {
		return requestAsync(req, whenLoaded, null);
	}
	 ...
	 */
	@SuppressLint("NewApi")
	public static HttpRequesterFuture requestAsync(final ExecutableRequest req, final HttpRequestListener whenLoaded,
			Activity activity) {
		...
		if (android.os.Build.VERSION.SDK_INT < android.os.Build.VERSION_CODES.HONEYCOMB) {
			...
			// Can be executed in a separate thread
			return new HttpRequesterFuture(
					(HttpRequesterTask) (new HttpRequesterTask(whenLoaded, null)).executeOnExecutor(
							AsyncTask.THREAD_POOL_EXECUTOR, loadReq));
		}
	}
	 ...
	 * 
	 * An instance of this class is returned for each async request that is executed.<br/>
	 * It can be used to query for the result of the execution in a synchronous manner (blocking operation), acting as a
	 * process barrier : When multiple async requests are executed in parallel, storing all these instances then
	 * querying for the {@link HttpRequesterFuture#get()} in a loop at some point in the program allows to guarantee
	 * that all requests have completed before going on. As this is a blocking operation that can last for some time, it
	 * is preferrable to do such things in a separate thread, or use the timed equivalent
	 * {@link HttpRequesterFuture#get(long, TimeUnit)}.
	 * <p>
	 * It is also possible to cancel a pending request through {@link HttpRequesterFuture#cancel(boolean)}.
	 * </p>
	 * <p>
	 * Note : A pending request being busy is bound to the life cycle of the underlying AsyncTask's
	 * {@link AsyncTask#doInBackground}, and does not extend to the {@link AsyncTask#onPostExecute}.
	 * </p>
	 * It is usually not a problem since {@link AsyncTask#onPostExecute} runs in the UI thread and as such should NOT do
	 * blocking stuffs anyway.
	 * 
	 * @author yoann@ymkm.org
	 */
	public static class HttpRequesterFuture implements Future<HttpResponseHolder> {
		...
		@Override
		public HttpResponseHolder get() throws InterruptedException, ExecutionException {
			return mRunningTask.get();
		}

		@Override
		public HttpResponseHolder get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException,
				TimeoutException {
	...
	private final static class HttpRequestHolder {
		...
		public final static HttpRequestHolder newInstance(ExecutableRequest http, HttpRequestListener listener) {
	...
	private final static class HttpRequesterTask extends AsyncTask<HttpRequestHolder, Void, HttpResponseHolder> {
		...
		protected void onCancelled() {
			if (null != mRequestListener) {
				mRequestListener.requestCancelled();
			}
		}
		...
		protected HttpResponseHolder doInBackground(HttpRequestHolder... reqs) {
			...
			if (null != mRequestListener) {
				mRequestListener.requestPrepare(req.http, Uri.parse(req.http.getURI().toString()));
			}
